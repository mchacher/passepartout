...
- **Album style**: give a project its own look with a font and a color palette (paper and text), from a small curated set. Seven typefaces, each shipped with the app and embedded in the exported PDF, so the printed book is set in the font you chose and not in a substitute. The accent color carries into the app so the album and the app read as one.
...
- **Arrange freely on a grid**: click a photo on a full page to arrange it. Layouts rest on a 12 x 12 page grid, and the mode lets you move and resize any photo on it, overlap photos and choose their front-to-back order. Escape, a click outside the page or Done leaves it. Photos are always contained, never cropped.
...
- **Decorative masks**: in Edit layout, give a photo a shape (circle, oval, rounded, arch, ...) from an enrichable catalog. The mask follows the photo's box and only hides the pixels outside the shape, so the photo keeps its aspect ratio and size. The circle stays a true circle and the rounded corners keep a constant radius whatever the photo's format (with three sizes to pick from). Opt-in and per-photo; no mask is the default.
- **Decorative frames**: mat a photo in a colored border (whole, never clipped, with a selectable width) or a Polaroid (a square window you can pan with Shift-drag, over a bottom band) from an enrichable color palette, and add a handwritten note on the Polaroid in a real handwriting font, on screen and in the exported PDF. When a photo also has a mask, the border follows the mask shape (a circle mask gives a circular border ring).
- **Notes on the page**: drop a small block of text anywhere on a page or a cover face, in the whitespace or over a photo. Drag it where you want it (it is gently attracted to the centre, the margins and the thirds), pull a handle to set where the text wraps, tilt it, and type on the paper itself. Six typefaces ship with the app, so the note prints exactly as previewed, line breaks included: a book serif, a display serif, a quiet sans, a rounded, a typewriter and a handwriting. Regular, italic or bold, five sizes, the album's inks or a colour of your own, and four typographic treatments (spaced small caps, a hairline rule, an opacity step, a paper reserve to stay legible over a photo). A note is an overlay: it never moves, resizes or crops a single photo.
